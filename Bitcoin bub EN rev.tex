...
\title{Bubbles and Rationality in Bitcoin}
\author{\textsf{George A. Waters} \\
...
\textit{On top of this fundamental demand, we can add a speculative demand.
...
behavior in the PCRB model. For an asset price determined by fundamentals $%
f_{t}$ and a bubble component $b_{t}$ such that $p_{t}=f_{t}+b_{t}$, the
bubble term in the PCRB model is as follows.%
\begin{eqnarray*}
b_{t} &=&\rho ^{-1}b_{t-1}v_{t}\text{ \ \ \ \ if }b_{t}\leq \alpha \\
b_{t} &=&\left[ \delta +\pi ^{-1}\rho ^{-1}\psi _{t}\left( b_{t-1}-\left(
1+r\right) ^{-1}\delta \right) \right] v_{t}\text{ \ \ \ \ if }b_{t}>\alpha
\end{eqnarray*}%
The parameter $\rho$ represents the discount factor where $0<\rho <1$ and $%
v_{t}$ is a stochastic variable with mean one. The stochastic term $\psi
...
are both positive and satisfy the condition $\delta <\left( 1+r\right)
\alpha $.
...
property $E_{t-1}\left( b_{t}\right) =\rho ^{-1}b_{t-1}$ is satisfied in
...
demand, it is rational to hold an asset in the explosive regime ($%
b_{t}>\alpha $) even if there is a possibility of collapse since there is
...
7/18/2010-2/27/2018\footnote{%
The sample has 2782 observations. All data is taken from coinmarketcap.com
with the exception of the mining difficulty series, which comes from
data.bitcoin.org.}. Table 1 reports results for the Augmented
Dickey-Fuller test on the bitcoin price $p$, the quantity of bitcoin
outstanding $q$, the difficulty of mining $diff$\footnote{%
The hash rate would be another measure of the cost of mining, but the data
is too limited for the tests reported here.}, the price of gold $p_{G}$, and
...
and Tsyvinski (2018)\footnote{%
Other references include Borri and Shakhnov (2018) who study bitcoin price
...
commodities.} use the number of bitcoin wallets as a funtdamental value.
...
and excess kurtosis of $u_{t}$. \medskip
...
estimated parameter is the $\widehat{\beta }$, while the statistics DF%
\footnote{%
Note that the ADF tests in Tables 1 and 2 differ since there are no lags $%
u_{t-1},u_{t-2},...$ included in the test reported in the latter.} and CR
test the null of non-cointegration $\widehat{\beta }=0$. The significance
probabilities are determined by Monte Carlo experiments\footnote{%
The DF and CR tests are computed with 20,000 simulations of a unit root with
drift for the price and dividend (if necessary) using coefficients estimated
with the bitcoin price data.} for the present sample analogous to those in
...
against \textit{i)}\ a\ constant, ii) a constant and a time trend and
\textit{iii)} the price of gold. The constant with or without the time
trend in \textit{i)}\ and \textit{ii)}\ represents the value of bitcoin as a
medium of exchange, which should be stable. The intuition behind \textit{%
iii)} is that bitcoin and gold are competing stores of value that do not
...
prices versus dividends for the S\&P 500 over more than a century. The $p$%
-values for the test including a time trend or with the price of gold are
even higher.
...
\begin{center}
{\Large Table 1}\bigskip

\begin{tabular}{|l|l|l|l|}
\hline
& ADF & $p$-value & lags \\ \hline
$p$ & -0.7663 & 0.8278 & 27 \\ \hline
$\Delta p$ & -8.7729 & 0.0000 & 27 \\ \hline
$\log p$ & -2.2162 & 0.2006 & 27 \\ \hline
$\Delta \log p$ & -20.8092 & 0.0000 & 27 \\ \hline
$q$ & -4.7854 & 0.0001 & 27 \\ \hline
$\Delta q$ & -2.3167 & 0.1667 & 27 \\ \hline
$\log q$ & -5.1116 & 0.0000 & 27 \\ \hline
$\Delta \log q$ & -4.6050 & 0.0001 & 27 \\ \hline
$p_{G}$ & -1.8940 & 0.3354 & 25 \\ \hline
$\Delta p_{G}$ & -43.0045 & 0.0000 & 25 \\ \hline
$\log p_{G}$ & -1.8093 & 0.3763 & 25 \\ \hline
$\Delta \log p_{G}$ & -43.0045 & 0.0000 & 25 \\ \hline
$\log diff$ & -3.3190 & 0.0142 & 27 \\ \hline
$\Delta \log diff$ & -7.4994 & 0.0000 & 27 \\ \hline
\end{tabular}%
\medskip
...
\hline
$\log p$ & $c$ & -0.00089 & & -0.00041 & \\ \hline
& & $\left( -2.6065\right)$ & 0.2805 & $\left( -1.4085\right)$ & 0.8572
\\ \hline
$\log p$ & $c,t$ & -0.0022 & & -0.00088 & \\ \hline
& & $\left( -2.5362\right)$ & 0.3126 & $\left( -1.2088\right)$ & 0.9070
\\ \hline
$\log p$ & $\log p_{G}$ & -0.0034 & & -0.0030 & \\ \hline
& & $\left( -2.7885\right)$ & 0.3076 & $\left( -2.5089\right)$ & 0.3265
\\ \hline
\end{tabular}%
\medskip
...
in Taylor and Peel (1998).\medskip }
\end{center}
